...
目标：WCAG AA 合规（对比度 ≥ 4.5:1 正文，≥ 3:1 大字/UI 元素）。
- 所有内置主题（包括创意主题）应尽量满足 AA；创意主题允许在注释中标注对比度说明
- 尊重 `prefers-reduced-motion`，动画默认关闭
- 焦点样式不得被覆盖为不可见

### Typography

默认字体栈：`system-ui, sans-serif`（跟随操作系统，零加载开销）。
- 未来可在设置面板提供用户自选字体（serif、等宽、自定义 Google Font）
- 中文内容需确保 CJK 字符渲染良好，line-height 1.6 对中文友好

### UI Interaction Pattern（工具栏/侧边栏）

**悬浮胶囊（Floating Pill）模式** — 高级感优先：
- **收起态**：屏幕边缘一条细长"椭圆/胶囊"快捷按钮，低干扰，有存在感
- **展开态**：悬浮弹出一个大椭圆/圆角面板，内容区保持干净
- 交互：鼠标靠近触发，或点击胶囊展开；点击外部/移开收起
